import React, { Component } from 'react';

class BottomBarHome extends Component {
  constructor(props) {
    super(props)
  }

  render() {
    return (
      <div className="bottom-bar-home">
          <img src="/img/profilepic1.jpg" />
        <p>Sleep. Meow go back to sleep owner brings food and water
          tries to pet on head, so scratch get sprayed by water because
          bad cat scratch the furniture so attack the child but be a
          nyan cat, feel great about it, be annoying 24/7 poop rainbows
          in litter box all day. Annoy kitten brother with poking try to
          jump onto window and fall while scratching at wall for freak
          human out make funny noise mow mow mow mow mow mow success now
          attack human poop in the plant pot taco cat backwards spells
          taco cat litter kitter kitty litty little kitten big roar roar
          feed me all of a sudden cat goes crazy. Lick arm hair. Really
          likes hummus.
        </p>
      </div>
    )
  }
}



export default BottomBarHome;
